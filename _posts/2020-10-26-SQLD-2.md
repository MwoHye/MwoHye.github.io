...
### 트랜젝션 제어어 (TCL) : COMMIT, ROLLBACK, SAVEPOINT
...
**★ Oracle와 SQL server의 COMMIT 명령어 차이** (https://yurimac.tistory.com/35)
...
```sql
UPDATE PLAYER SET HEIGHT = 100; // Oracle
COMMIT; 

UPDATE PLAYER SET HEIGHT = 100; // SQL
```
...
### SAVEPOINT0 : 현 시점에서 SAVEPOINT까지 트렌젝션 일부를 ROLLBACK
```sql
SAVEPOINT 세이브포인트이름;
...
### SQL 연산자 : IN 연산자, LIKE 연산자, BETWEEN a AND b 연산자, IS NULL 연산자
...
```sql
SELECT POSITION, HEIGHT FROM PLAYER WHERE POSITION LIKE 'MF';
SELECT PORISION, HEIGHT FROM PLATER WHERE NAME LIKE '김%';
```
- 와일드카드
  - % : 0개 이상의 어떤 문자
  - _ : 1개의 단일 문자
...
```sql
SELECT NAME 선수이름, POSITION 포지션 FROM PLAYER WHERE POSITION IS NULL;
```
- ★ NULL 사칙연산 연산 : NULL VALUE 리턴
...
### p57 38번문제 : 2014년 11월부터 2015년 3월까지 매출금액 합계를 출력하기
...
2. 내장 함수 : 입력되는 값이 많아도 출력은 하나만 (M:1)
- LOWER, UPPER, ASCII, CONCAT, SUBSTR, LEN, LTRIM(RTRIM,TRIM), RTRIM

3. 문자형 함수
- (60p 41번) REPLACE(C1, CHR(10)) : 줄바꿈을 empty string으로 바꿔줌
...
5. 날짜형 함수
6. 변환형 함수
7. CASE 함수 : IF-THEN-ELSE (61p 43번)
...
- NVL(표현식1, 표현식2) / ISNULL(표현식1, 표현식2)
  - 표현식1 결과값이 NULL이면 표현식2 리턴
- NULLIF(표현식1, 표현식2)
  - 표현식1 = 표현식2 이면 NULL, 같지 않으면 표현식1 리턴
- COALESCE(표현식1, 표현식 2 ...)
  ```sql
  SELECT ENAME, COMM, COALESCE(COMM, SAL) COAL FROM EMP;
  ```
  - 여러 표현식에서 NULL이 아닌 최초의 표현식을 리턴, 모든 표현식이 NULL이면 NULL 리턴
